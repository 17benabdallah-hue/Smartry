...
export function detectEventType(text: string): EventType {
  const lowerText = text.toLowerCase();
  if (/(رحلة|طائرة|مطار|سفر|flight|airport|travel)/.test(lowerText)) return EventType.FLIGHT;
  if (/(اجتماع|موعد|لقاء|مقابلة|meeting|appointment)/.test(lowerText)) return EventType.MEETING;
  if (/(دواء|علاج|حبة|medicine|pill|medication)/.test(lowerText)) return EventType.MEDICINE;
  if (/(حليب|طعام|فرن|طبخ|اكل|milk|food|cook)/.test(lowerText)) return EventType.FOOD;
  if (/(مدرسة|ابن|ابنة|طفل|school|child|kid)/.test(lowerText)) return EventType.SCHOOL;
  if (/(سفر|رحلة|travel|trip)/.test(lowerText)) return EventType.TRAVEL;
