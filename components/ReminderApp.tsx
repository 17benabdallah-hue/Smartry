        ...
        if (r.id === id) {
          ...
          if (notificationService) {
            if (newState) {
              ...
              notificationService.scheduleReminder(r, (rid) => handleReminderDue(rid));
            }
          }
    ...
    const interval = setInterval(() => {
      setReminders(prev => {
        ...
        if (dueReminders.length > 0) {
          dueReminders.forEach(rem => handleReminderDue(rem.id));
        }
        return prev;
      });
    }, 10000);
    ...
    <div className="min-h-screen bg-[#E65100] dark:bg-zinc-950 flex flex-col transition-colors duration-500">
              ...
              {activeReminders.length === 0 ? (
                ...
                activeReminders.map((rem) => {
                  const isDueSoon = !rem.isCompleted && 
                    new Date(rem.reminderTime).getTime() - new Date().getTime() < 300000 && // 5 mins
                    new Date(rem.reminderTime).getTime() - new Date().getTime() > 0;
      ...
      <footer className="py-8 text-center bg-black/5">
        <p className="text-[10px] font-black text-white/30 uppercase tracking-[0.3em]">
          {t.app_name} © 2026 • عبدالله بن عبدالله
        </p>
      </footer>
    </div>
  );
